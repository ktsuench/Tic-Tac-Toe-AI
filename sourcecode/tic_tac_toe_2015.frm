...
Public Function dir(ByVal sdir As Integer, ByVal cond As Integer, ByVal inc As Integer)
    ...
    Do While res < 0 And i <= cond And res <> -2
        ...
        Else
            If Len(x) > 0 Then
                If Len(y) > 0 Then If x = y Then res = IIf(sdir = 1, i + 1, IIf(sdir = 2, i + 5, 7 + k))
                If Len(z) > 0 Then If x = z Then res = IIf(sdir = 1, i, IIf(sdir = 2, i + 2, 4))
            ElseIf Len(y) > 0 And Len(z) > 0 Then
                If y = z Then res = IIf(sdir = 1 Or sdir = 2, i - 1, 1 - k)
            End If
        End If
